...
object Driver extends App{
  if(args.length != 2){
    print("Usage: Q3Driver <input dir> <output dir>")
    System.exit(-1)
  }
  ...
  job.setMapperClass(classOf[Q3Mapper])
  job.setReducerClass(classOf[Q3Reducer])
  job.setMapOutputKeyClass(classOf[Text])
  job.setMapOutputValueClass(classOf[MaxLinkWritable])

  // Specify the job's output key and value classes
  job.setOutputKeyClass(classOf[Text])
  job.setOutputValueClass(classOf[MaxLinkWritable])
